...
class FMIndex(object):
    ...
    def create_suffix_array(self, text_list):
        """create suffix array representation of given text"""
        suffix_refs = []
        for read_num in xrange(len(text_list)):
            addtl_refs = [(read_num, offset) for offset in (xrange(len(text_list[read_num])))]
            suffix_refs.extend(addtl_refs)
    ...
    def ref_comp(self, x, y):
        ...
        i = 0
        while self.text[x[0]][x[1]+i] == self.text[y[0]][y[1]+i]:
            if self.text[x[0]][x[1]+i] == '$':
                return x[0] - y[0]
            i += 1
